...
import { SearchResultCard } from '@/features/trade/search-post/ui/SearchResultCard';
import { SearchSkeletonCard } from '@/features/trade/search-post/ui/SearchSkeletonCard';
...
export const SearchResult = ({ search, posts, isLoading, isError }: SearchResultProps) => {
  ...
  if (!posts || posts.length === 0) {
    return (
      <EmptyState
        title="검색 결과가 없습니다"
        description={`"${search}"에 대한 결과를 찾을 수 없어요`}
      />
    );
  }
